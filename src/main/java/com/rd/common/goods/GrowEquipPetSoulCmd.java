 ...
 * @author ---
 * @version 1.0
 * @date 2018年5月17日下午2:06:00
 */
public class GrowEquipPetSoulCmd implements IGoodsCmd {

    private static final GrowEquipPetSoulCmd _instance = new GrowEquipPetSoulCmd();

    public static GrowEquipPetSoulCmd gi() {
        return _instance;
    }

    private GrowEquipPetSoulCmd() {
    }

    @Override
    public long getValue(GameRole role, DropData data) {
        return role.getPlayer().getGrowEquipList().get(EEquip.PET_SOUL.ordinal()).get(data.getG());
    }
    ...
    public boolean consume(GameRole role, DropData data, EGoodsChangeType changeType, EnumSet<EPlayerSaveType> enumSet) {
        if (data.getN() < 1)
            return false;
        Integer goods = role.getPlayer().getGrowEquipList().get(EEquip.PET_SOUL.ordinal()).get(data.getG());
        if (goods != null && goods >= data.getN()) {
            goods -= data.getN();
            if (goods <= 0) {
                role.getPlayer().getGrowEquipList().get(EEquip.PET_SOUL.ordinal()).remove(data.getG());
            }
    ...
    public boolean reward(GameRole role, DropData data, EGoodsChangeType changeType, EnumSet<EPlayerSaveType> enumSet) {
        ...
        goods += data.getN();
        role.getPlayer().getGrowEquipList().get(EEquip.PET_SOUL.ordinal()).put(data.getG(), goods);
...
//		}
    ...
    public void sendAddItemMsg(GameRole role, short d, int n, EGoodsChangeType changeType) {
        ...
        //msg.setByte(EGoodsType.GROW_EQUIP_PET_SOUL.getId());
        msg.setShort(d);
        msg.setInt(n);

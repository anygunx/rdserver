 ...
 */
public class GrowEquipCmd implements IGoodsCmd {
    private EEquip type;

    public GrowEquipCmd(EEquip type) {
        this.type = type;
    }

    @Override
    public long getValue(GameRole role, DropData data) {
        return role.getPlayer().getGrowEquipList().get(type.ordinal()).get(data.getG());
    }
    ...
    public boolean consume(GameRole role, DropData data, EGoodsChangeType changeType, EnumSet<EPlayerSaveType> enumSet) {
        if (data.getN() < 1)
            return false;
        Integer goods = role.getPlayer().getGrowEquipList().get(type.ordinal()).get(data.getG());
        if (goods != null && goods >= data.getN()) {
            goods -= data.getN();
            if (goods <= 0) {
                role.getPlayer().getGrowEquipList().get(type.ordinal()).remove(data.getG());
            }
    ...
    public boolean reward(GameRole role, DropData data, EGoodsChangeType changeType, EnumSet<EPlayerSaveType> enumSet) {
        ...
        goods += data.getN();
        role.getPlayer().getGrowEquipList().get(type.ordinal()).put(data.getG(), goods);

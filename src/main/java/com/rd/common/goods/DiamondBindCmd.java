...
public class DiamondBindCmd implements IGoodsCmd {

    private static final Logger logger = Logger.getLogger(DiamondBindCmd.class);

    private static final DiamondBindCmd _instance = new DiamondBindCmd();

    public static DiamondBindCmd gi() {
        return _instance;
    }

    private DiamondBindCmd() {
    }

    @Override
    public long getValue(GameRole role, DropData data) {
        return role.getPlayer().getBindDiamond();
    }
    ...
    public boolean consume(GameRole role, DropData data, EGoodsChangeType changeType, EnumSet<EPlayerSaveType> enumSet) {
        ...
        if (!validate(role, data))
            return false;
        int value = -data.getN();
        role.getPlayer().changeBindDiamond(value);
        //每日消耗元宝数
...
//		}
        //记录玩家元宝变化日志
        LogUtil.log(role.getPlayer(), new MoneyChange(EGoodsType.BINDDIAMOND.getId(), value, changeType.getId()));
        logger.info(role.getPlayerId() + "_" + role.getPlayer().getBindDiamond());

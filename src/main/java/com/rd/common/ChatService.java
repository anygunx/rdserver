 ...
 * 聊天服务类
 *
 * @author Created by U-Demon on 2016年11月8日 下午1:46:19
 * @version 1.0.0
 */
public class ChatService {
    ...
    public static void addChatMsg(Chat chat) {
        if (chatQueue.size() >= CHAT_CACHE_CAPACITY)
            chatQueue.poll();
        chatQueue.offer(chat);
    }

    public static void addGangChatMsg(Chat chat) {
        ...
        if (queue.size() >= CHAT_CACHE_CAPACITY)
            queue.poll();
        queue.offer(chat);
    }
     ...
     * @return
     */
    public static Message getChatListMsg() {
        Message msg = new Message(MessageCommand.CHAT_LIST_MESSAGE);
    ...
    public static Message getGangChatListMsg(int gangId) {

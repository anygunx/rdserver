...
public class TLShopEvent implements IActivityEvent {
    ...
    public void getMessage(Message msg, GameRole role) {
        ...
        BaseActivityConfig configData = ActivityService.getActivityConfig(EActivityType.TLSHOP);
        ActivityRoundConfig currRound = configData.getCurrRound(0, curr);
        if (currRound == null)
            return;
        ActivityGroupData group = ActivityService.getGroupData(EActivityType.TLSHOP);
        //活动数据轮次
        msg.setByte(group.getDataRound(currRound.getRound()));
        PlayerActivity activityData = role.getActivityManager().getActivityData();
        Map<Integer, Integer> buys = activityData.getShopBuy(EActivityType.TLSHOP);

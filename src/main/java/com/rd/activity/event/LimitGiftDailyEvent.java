 ...
 * @version 1.0.0
 */
public class LimitGiftDailyEvent implements IActivityEvent {
    ...
    public void getMessage(Message msg, GameRole role) {
        ...
        BaseActivityConfig configData = ActivityService.getActivityConfig(EActivityType.FEST_LIMIT_DAILY_GIFT);
        ActivityRoundConfig currRound = configData.getCurrRound(0, curr);
        if (currRound == null)
            return;
        ActivityGroupData group = ActivityService.getGroupData(EActivityType.FEST_LIMIT_DAILY_GIFT);
        //活动数据轮次
        msg.setByte(group.getDataRound(currRound.getRound()));
        PlayerActivity activityData = role.getActivityManager().getActivityData();
        Map<Integer, Integer> buys = activityData.getGiftBuy(EActivityType.FEST_LIMIT_DAILY_GIFT);

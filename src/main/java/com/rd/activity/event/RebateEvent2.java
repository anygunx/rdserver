 ...
 * @version 1.0.0
 */
public class RebateEvent2 implements IActivityEvent {

    private static Logger logger = Logger.getLogger(RebateEvent2.class);

    @Override
    public boolean onStart() {
        logger.info("节日特惠包活动开启!");
        return true;
    }

    @Override
    public boolean onEnd() {
        logger.info("节日特惠包活动结束!");
        new ActivityDao().clearRebate(EActivityType.FEST_REBATE);
        for (GameRole role : GameWorld.getPtr().getOnlineRoles().values()) {
            ...
            for (int i = activityData.getRebates().size() - 1; i >= 0; i--) {
                if (activityData.getRebates().get(i) / 10000 == EActivityType.FEST_REBATE.getId())
    ...
    public void getMessage(Message msg, GameRole role) {
        ...
        for (int key : activityData.getRebates()) {
            if (key / 10000 == EActivityType.FEST_REBATE.getId()) {
                list.add(key % 10000);
            }
        }
    ...
    public boolean isOpen(GameRole role) {
        ...
        Map<String, RebateLogicData> logicData = ActivityService.getRoundData(
                EActivityType.FEST_REBATE, role.getPlayerId(), System.currentTimeMillis());
        List<Integer> list = new ArrayList<>();
        for (int key : activityData.getRebates()) {
            if (key / 10000 == EActivityType.FEST_REBATE.getId()) {
                list.add(key % 10000);

 ...
 * @version 1.0.0
 */
public class RebateEventN implements IActivityEvent {

    private static Logger logger = Logger.getLogger(RebateEventN.class);
    ...
    public boolean onEnd() {
        ...
        new ActivityDao().clearRebate(EActivityType.REBATE_N);
        for (GameRole role : GameWorld.getPtr().getOnlineRoles().values()) {
            ...
            for (int i = activityData.getRebates().size() - 1; i >= 0; i--) {
                if (activityData.getRebates().get(i) / 10000 == EActivityType.REBATE_N.getId())
    ...
    public void getMessage(Message msg, GameRole role) {
        ...
        for (int key : activityData.getRebates()) {
            if (key / 10000 == EActivityType.REBATE_N.getId()) {
                list.add(key % 10000);
            }
        }
    ...
    public boolean isOpen(GameRole role) {
        ...
        Map<String, RebateLogicData> logicData = ActivityService.getRoundData(
                EActivityType.REBATE_N, role.getPlayerId(), System.currentTimeMillis());
        List<Integer> list = new ArrayList<>();
        for (int key : activityData.getRebates()) {
            if (key / 10000 == EActivityType.REBATE_N.getId()) {
                list.add(key % 10000);

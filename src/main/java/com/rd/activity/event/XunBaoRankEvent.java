...
public class XunBaoRankEvent implements IActivityEvent {

    private static final Logger logger = Logger.getLogger(XunBaoRankEvent.class);
    ...
    public boolean onEnd() {
        ...
        Map<String, XunBaoRankLogicData> logicData = ActivityService.getRoundData(
                EActivityType.XUNBAO_RANK, 0, System.currentTimeMillis() - 3600000);
        if (logicData == null)
            return false;
    ...
    public void getMessage(Message msg, GameRole role) {
        ...
        BaseActivityConfig configData = ActivityService.getActivityConfig(EActivityType.XUNBAO_RANK);
        ActivityRoundConfig currRound = configData.getCurrRound(0, curr);
        if (currRound == null)
            return;
        ActivityGroupData<TargetLogicData> group = ActivityService.getGroupData(EActivityType.XUNBAO_RANK);
        //活动数据轮次
